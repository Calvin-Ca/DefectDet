...
def train(hyp, opt, device, callbacks):
    ...
    opt.hyp = hyp.copy()  # for saving hyps to checkpoints,opt:options

    # Save run settings
    if not evolve:
        yaml_save(save_dir / "hyp.yaml", hyp)       # hyp超参数
        yaml_save(save_dir / "opt.yaml", vars(opt)) # opt训练配置
    ...
    if pretrained:
        ...
        # .float()统一成 FP32,防止：AMP\半精度 checkpoint
        csd = ckpt["model"].float().state_dict()  # checkpoint state_dict as FP32
        # 权重“求交集”加载（YOLOv5 的精华）
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect
        model.load_state_dict(csd, strict=False)  # load
        LOGGER.info(f"Transferred {len(csd)}/{len(model.state_dict())} items from {weights}")  # report
    else:
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # create
    amp = check_amp(model)  # check AMP,AMP（自动混合精度）检测
    ...
    # Image size
    # model.stride 是一个 Tensor / list,表示 每个检测头对应的下采样倍率
    ...
    # Optimizer
    nbs = 64  # nominal batch size,YOLOv5 设计时假定的“基准 batch size”
    ...
    # EMA,指数滑动平均维护一份 “更平滑的模型权重”,验证 / 保存时用 EMA，而不是当前模型,EMA 几乎总是比原模型更稳，mAP 更高
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            if not opt.noautoanchor:
                # （首次训练）根据数据重新聚类 anchors 非常重要，对收敛速度影响极大
                check_anchors(dataset, model=model, thr=hyp["anchor_t"], imgsz=imgsz)  # run AutoAnchor
            model.half().float()  # pre-reduce anchor precision,减少 anchor 数值误差,再转回 FP32
...
def parse_opt(known=False):     # known 为 True,只解析我认识的参数，其他的先不管,known 为 False,所有参数必须合法，不然就报错退出
    ...
    # 多尺度训练,每个 batch 随机调整图片尺寸 ±50%
    parser.add_argument("--multi-scale", action="store_true", help="vary img-size +/- 50%%")
    # 多分类数据当作单分类训练,常用于目标检测 + 只有“目标 / 非目标”
    parser.add_argument("--single-cls", action="store_true", help="train multi-class data as single-class")
    parser.add_argument("--optimizer", type=str, choices=["SGD", "Adam", "AdamW"], default="SGD", help="optimizer")
    # 多 GPU 同步 BatchNorm,仅 DDP 模式有效,Distribution Data Parallel
    ...
    # Logger arguments
    # 「日志 / 实验追踪」参数，主要服务于 Weights & Biases（W&B） 这一套实验管理系统
    ...
    # NDJSON logging
    # Newline Delimited JSON,一行一个JSON,每条日志独立
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        # check_comet_resume,是否使用 Comet ML 云端实验恢复
        # 是否做超参数进化(evolve 模式下不允许resume)
